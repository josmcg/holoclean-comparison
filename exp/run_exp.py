...
from repair.featurize import InitAttrFeaturizer
from repair.featurize import InitSimFeaturizer
from repair.featurize import FreqFeaturizer
from repair.featurize import OccurAttrFeaturizer as OccurFeaturizer
from repair.featurize import ConstraintFeaturizer as ConstraintFeat
from repair.featurize import LangModelFeaturizer as LangModelFeat
...
hc = holoclean.HoloClean(db_host='db', pruning_topk=0.1, epochs=12, weight_decay=0.01, threads=20, batch_size=1, verbose=True,
                         timeout=3 * 60000).session

# 2. Load training data and denial constraints.

hc.load_data('hospitalk',  'data/hospital.csv')
hc.load_dcs( 'data/hospital_constraints.txt')
...
hc.setup_domain()
featurizers = [InitAttrFeaturizer(), InitSimFeaturizer(), FreqFeaturizer(), OccurFeaturizer(), LangModelFeat(),
               ConstraintFeat()]
...
hc.evaluate(fpath="data/hospital_clean.csv",
            tid_col="tid",
            attr_col="attribute",
            val_col="correct_val"
        )
